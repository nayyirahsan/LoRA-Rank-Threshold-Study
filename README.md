...
> Status: **complete for this pass.** LR calibration (27 runs), the main grid (55 runs), and the SVD-oracle
> sweep (66 oracle evals over 6 full-FT runs) ran on free Kaggle 2×T4 sessions. Cut for budget: facts
> N=16k, extra facts seeds, the rsLoRA/QLoRA ablations, and the 1.7B check.
...
| H2a, energy rank predicts r* | **fails** | 0/4 settings within 2×. facts N=1,000: 90%-energy rank 192 vs r* 1; sql N=2,000: 90%-energy rank 181 vs r* 1; facts N=250: 90%-energy rank 127 vs r* 1; facts N=4,000: 90%-energy rank 268 vs r* 1 |

**H2b, oracle truncation vs trained LoRA:**

- Facts N=250: mean G(LoRA) − G(oracle) = +0.57 over ranks 1, 2, 4, 8, 16, 64, 256 → LoRA ≫ oracle: compact rank-r solutions exist that full FT didn't pick
- Facts N=1,000: mean G(LoRA) − G(oracle) = +0.63 over ranks 1, 2, 4, 8, 16, 64, 256 → LoRA ≫ oracle: compact rank-r solutions exist that full FT didn't pick
- Facts N=4,000: mean G(LoRA) − G(oracle) = +0.70 over ranks 1, 2, 4, 8, 16, 64, 256 → LoRA ≫ oracle: compact rank-r solutions exist that full FT didn't pick
- SQL n=2,000: mean G(LoRA) − G(oracle) = +0.24 over ranks 1, 2, 4, 8, 16, 32, 64, 128 → LoRA ≫ oracle: compact rank-r solutions exist that full FT didn't pick

**Resume bullet** (generated from the numbers above):

> Designed and ran a 55-run LoRA rank × data-size study on Qwen3-0.6B against full fine-tuning on free Kaggle T4s; found LoRA recovers ≥95% of full fine-tuning's gain at rank 1 (the smallest tested) on text-to-SQL and rank 1 (the smallest tested) for 4,000 injected facts, and that trained LoRA beats SVD-truncated full-FT updates of equal rank by 0.54 mean gap closure: low-rank solutions exist that full fine-tuning doesn't find.

### Thresholds

| Task | N | Base | Full FT | r* LoRA | r* LoRA (CI-strict) | r* oracle | 90%-energy rank of ΔW | Prediction / r* LoRA | Seeds |
|---|---|---|---|---|---|---|---|---|---|
| facts | 250 | 0.000 | 1.000 | 1 | 1 | 64 | 127 | 127.00× (outside 2×) | 1 |
| facts | 1,000 | 0.000 | 1.000 | 1 | 1 | 64 | 192 | 192.00× (outside 2×) | 1 |
| facts | 4,000 | 0.000 | 0.999 | 1 | 1 | 128 | 268 | 268.00× (outside 2×) | 1 |
| sql | 2,000 | 0.318 | 0.859 | 1 | 1 | 32 | 181 | 181.00× (outside 2×) | 3 |

![Gap closure vs rank: trained LoRA and the SVD oracle, per setting](figures/gap_closure.png)

![Share of the full-FT update's energy in its top-r directions](figures/spectrum.png)
<!-- results:end -->

## What the results mean

**Rank-1 LoRA matched full fine-tuning on every task and data size tested.** On SQL that fits H1
(r\* ≤ 8). On facts it means capacity never became the limit, so the predicted growth of r\* with N
can't be observed. A capacity estimate explains why ([log #18](docs/engineering_log.md)):

- A rank-1 adapter on all 7 projections of Qwen3-0.6B has **630,784** trainable parameters.
- 4,000 facts carry **≈ 26 kbits** (6.58 bits per fact, from the attribute pool sizes).
- At the ~1–2 bits/parameter knowledge capacity measured in *Physics of Language Models 3.3*, rank 1
  holds **24–48× more than needed**. Capacity should only start to bind near **~10⁵ facts**.

That turns the null result into a testable prediction. Scaling N to ~10⁵ doesn't fit free-tier
compute, so the next experiment shrinks the adapter instead. LoRA r=1 on a single projection in one
layer (~3–4k parameters) should hit its knee at a few hundred to a thousand facts, a few GPU-hours of work.

**The oracle shows where the predicted growth actually lives: in full fine-tuning's update, not in
what the task needs** ([log #19](docs/engineering_log.md)).

| Facts N | Rank-1 LoRA | Rank-1 truncation of full FT's update | Oracle r\* | 90%-energy rank of ΔW |
|---|---|---|---|---|
| 250 | 0.964 | 0.020 | 64 | 127 |
| 1,000 | 1.000 | 0.000 | 64 | 192 |
| 4,000 | 0.998 | 0.003 | 128 | 268 |

As facts grow 16×, full FT spreads its update over more directions: the energy rank roughly doubles and
the oracle threshold rises from 64 to 128. That's the growth H1 predicted. But LoRA shows no such growth,
because rank 1 was always enough. Full FT isn't finding a low-rank solution and adding noise on top: its
top singular direction alone recovers essentially nothing. The practical reading is that **the effective
rank of a full fine-tuning update overstates the rank an adapter needs**. Arguing "LoRA needs high rank
because full-FT deltas are high-rank" doesn't hold here. H2a as stated fails for the same reason: the
energy rank predicts the *oracle's* threshold (2–3× high on facts, 5.7× on SQL), not LoRA's.

**Caveats:** the facts results are 1 seed each (SQL has 3). The capacity law was measured on full
models, not adapters, so the estimate is order-of-magnitude only. The one high-rank dip (facts N=250,
r=256: 0.932) is consistent with α/r underfitting, but the rsLoRA ablation that would test it was cut.
The retrained full-FT runs the oracle used reproduced the originals exactly on facts and within ±0.004 on SQL.

**Resume bullet:**

> Ran a 55-run LoRA rank × data-size study on Qwen3-0.6B (free Kaggle T4s) with an SVD-truncation oracle of
> the full fine-tuning update; found rank-1 LoRA recovers ≥95% of full fine-tuning on text-to-SQL and up to
> 4,000 injected facts, while rank-1 truncations of the full-FT update recover ≤2%, showing the update's
> effective rank (oracle threshold 64→128 as data grows 16×) overstates the rank LoRA needs.
...
scripts/
  run_grid.py       expand a YAML grid; FT-first ordering; --dry-run / --limit / --shard; survives failed configs
  run_parallel.py   one shard per GPU, waits for all (works in batch kernels)
  run_oracle.py     spectrum + oracle rank sweep for one full-FT checkpoint
  kaggle_launch.py  push / status / watch / pull a grid as a private 2×T4 Kaggle kernel
  estimate_grid.py  GPU-hour estimate for a grid from measured throughput
  aggregate.py      registry → results/curves.csv, r_star.md, figures/*.png
  write_results.py  registry → README results section with computed hypothesis verdicts
  audit_sqlexec.py  false-positive audit of the SQL matcher on real gold queries
kaggle/             batch kernel template (preflight, memory gate, grid, in-kernel oracle, aggregate)
configs/            lr_cal, lr_cal_ext (calibration); grid_final, grid_final_oracle (study);
                    grid_sql, grid_facts, ablations (original full-scope plan); smoke
results/            kaggle/<grid>/ raw kernel outputs; final/ the analysis registry and tables
docs/engineering_log.md
tests/              101 tests: data invariants, matcher, registry, oracle sanity, LoRA init/param count,
                    micro-batching, analysis, verdict logic, launcher
```
